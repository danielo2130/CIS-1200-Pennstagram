...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                tgt[col + borderWidth][row + borderWidth] = src[col][row];
            }
        }
        //LHS Border
        ...
        for (int col = 0; col < borderWidth; col++) {
            for (int row = 0; row < h + 2; row++) {
                tgt[w + borderWidth + col][row] = borderColor;
            }
        }
        // Bottom Border
        for (int col = 0; col < borderWidth; col++) {
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int blue = p.getBlue();
                bmp[row][col] = new Pixel(255 - red, 255 - green, 255 - blue);
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[col][row];
                int red = (int) Math.round(p.getRed() * rFactor);
                int green = (int) Math.round(p.getGreen() * gFactor);
                int blue = (int) Math.round(p.getBlue() * bFactor);
    ...
    public static PixelPicture alphaBlend(
        ...
        if (picwidth == fwidth && picheight == fheight) {
            Pixel[][] picBMP  = pic.getBitmap();
            Pixel[][] fBMP = f.getBitmap();
            for (int col = 0; col < picwidth; col++) {
                for (int row = 0; row < picheight; row++) {
                    Pixel picp = picBMP[col][row];
                    Pixel fp = fBMP[col][row];
                    int red = weightedAverage(alpha, picp.getRed(), fp.getRed());
                    int green = weightedAverage(alpha, picp.getGreen(), fp.getGreen());
                    int blue = weightedAverage(alpha, picp.getBlue(), fp.getBlue());
                    picBMP[col][row] = new Pixel(red, green, blue);
                }
            }
